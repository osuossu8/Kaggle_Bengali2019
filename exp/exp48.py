...
from src.model import CnnModel, CnnModelV2, CNNHead
...
data_transforms_96 = albumentations.Compose([
    albumentations.ShiftScaleRotate(shift_limit=0.0625, scale_limit=0.1, rotate_limit=(5,15), p=0.5),
...
data_transforms_104 = albumentations.Compose([
    albumentations.ShiftScaleRotate(shift_limit=0.0625, scale_limit=0.1, rotate_limit=(5,15), p=0.3),
    albumentations.CenterCrop(104, 104, p=1),
    albumentations.Resize(256, 256, p=1),
    albumentations.Cutout(p=0.3),
    albumentations.OneOf([
            GridMask(num_grid=2, rotate=(5, 15), p=0.3),
            GridMask(num_grid=3, rotate=(5, 15), p=0.3),    
        ], p=0.3)
    ])


data_transforms_test_96 = albumentations.Compose([    
    albumentations.CenterCrop(96, 96, p=1),
    albumentations.Resize(256, 256, p=1),                                      
    ])

data_transforms_test_104 = albumentations.Compose([    
    albumentations.CenterCrop(104, 104, p=1),
    albumentations.Resize(256, 256, p=1),                                      
    ])

data_transforms = [data_transforms_96, data_transforms_104]
data_transforms_test = [data_transforms_test_96, data_transforms_test_104]
...
batch_size_list = [8, 36, 42, 48, 52, 64]
...
with timer('load csv data'):
    ...
    train_idx, val_idx = train_test_split(train.index.tolist(), test_size=0.1, random_state=SEED, stratify=train["grapheme_root"])    
...
with timer('create model'):
    ...
    model.features.conv0 = nn.Conv2d(1, 64, kernel_size=7, stride=2, padding=3,
                               bias=False)
    model.features.conv0.weight = nn.Parameter(torch.mean(w, dim=1, keepdim=True))
    model.classifier = CNNHead(model.classifier.in_features)
