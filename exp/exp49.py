...
from src.model import Efficient
...
EXP_ID = "exp49_256_efficient0_over9000_gem_90epoch_2e-3_exp47_weight"
...
batch_size_list = [12, 16, 18, 32, 46, 64, 72, 128]
...
with timer('load csv data'):
    ...
    # train_idx, val_idx = train_test_split(train.index.tolist(), test_size=0.15, random_state=SEED, stratify=train["grapheme"])

    print(len(train_idx), len(val_idx))

    for lbl in mis_label_index:
        if lbl in train_idx:
            train_idx = np.delete(np.array(train_idx), lbl)
        if lbl in val_idx:
            val_idx = np.delete(np.array(val_idx), lbl)
...
with timer('create model'):
    ...
    model = Efficient(186, encoder='efficientnet-b0', pool_type="gem")

    # model = convert_model_ReLU2Mish(model)
    model.load_state_dict(torch.load("models/exp47_efficient_over9000_gem_sum_conv_weight_75epoch_4e-4_fold0.pth"))
    model = model.to(device)

    criterion = nn.CrossEntropyLoss(reduction='mean').to(device)
    # optimizer = Over9000(model.parameters(), lr=2e-3, weight_decay=1e-3) ## New once
    optimizer = Over9000(model.parameters(), lr=2e-4, weight_decay=1e-4) ## New once
