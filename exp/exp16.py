...
from src.image_bengali import rand_bbox, cutmix, mixup, cutmix_criterion, mixup_criterion, cutmix_criterion_focal, mixup_criterion_focal
from src.trainer_bengali import train_one_epoch_mixup_cutmix, train_one_epoch_with_focalloss, validate_with_focalloss
...
EXP_ID = "exp16_mixup_cutmix_45epoch_4e-4_focalloss_64px"
...
class BengaliAIDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        image = self.df.iloc[idx][1:].values.reshape(self.size,self.size).astype(np.float)
...
with timer('load feather data'):
    train_path = [
        'input/resize_cropped_64_train_image_data_0.feather',
        'input/resize_cropped_64_train_image_data_1.feather',
        'input/resize_cropped_64_train_image_data_2.feather',
        'input/resize_cropped_64_train_image_data_3.feather'
    ]
...
with timer('training loop'):
    ...
    for epoch in range(1, epochs + 1):
        ...
        tr_loss = train_one_epoch_with_focalloss(model, train_loader, criterion, optimizer, device)
        LOGGER.info('Mean train loss: {}'.format(round(tr_loss, 5)))

        val_pred, y_true, val_loss = validate_with_focalloss(model, val_loader, criterion, device)
