...
from src.model import Efficient, CnnModelV2, CNNHead6th, BengaliResNeXt
...
EXP_ID = "exp64_late_not_validate"
...
data_transforms = albumentations.Compose([
    ...
    albumentations.PadIfNeeded(min_height=140, min_width=245, p=1.0),
    albumentations.Rotate(limit=5),
    albumentations.Resize(128, 224, p=1),
...
batch_size_list = [16, 20, 24, 42, 64, 72]
...
with timer('prepare validation data'):
    ...
    y_train = y
    train_dataset = BengaliAIDataset(data, y=y_train, transform=data_transforms)
...
with timer('training loop'):
    ...
    for epoch in range(1, epochs + 1):
        ...
        #     best_score = score
        #     best_epoch = epoch
        if epoch in [35, 70, 105]:
            torch.save(model.state_dict(), os.path.join(OUT_DIR, '{}_fold{}_at{}.pth'.format(EXP_ID, fold_id, epoch)))
            ...
            LOGGER.info("save model on epoch={}".format(best_score, best_epoch))
        scheduler.step()
